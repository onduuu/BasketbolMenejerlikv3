...
class Tournament {
    ...
    showMatchResult(result) {
        // Maç sonuç overlay'ini oluştur
        const overlay = document.createElement('div');
        overlay.className = 'match-result-overlay';
        
        const resultBox = document.createElement('div');
        resultBox.className = 'match-result-box';
        
        // Kazanma/kaybetme durumuna göre içerik ve stil
        const isWin = result.homeScore > result.awayScore;
        const resultText = isWin ? 'KAZANDINIZ!' : 'KAYBETTİNİZ';
        ...
        resultBox.innerHTML = `
            ...
            <div class="rewards">
                <div class="xp-reward">
                    <i class="fas fa-star"></i> +25 XP
                </div>
                <div class="money-reward">
                    <i class="fas fa-dollar-sign"></i> +$100,000
                </div>
            </div>
            ` : ''}
            <button class="confirm-button">TAMAM</button>
        `;
        ...
        const confirmButton = resultBox.querySelector('.confirm-button');
        confirmButton.addEventListener('click', () => {
            if (isWin) {
                // XP ve para ödülünü ver
                playerXP += 25;
                money += 100000;
                ...
                checkMissionProgress({
                    ...
                    scoreDifference: result.homeScore - result.awayScore
                });
            }
            ...
            // Turnuvalar ekranına dön
            document.getElementById('match-screen').style.display = 'none';
            document.getElementById('tournament-selection').style.display = 'block';
        });
    }

    // Maç istatistiklerini kaydetme
    saveMatchStats(matchResult) {
        let matchStats = JSON.parse(localStorage.getItem('matchStats') || '[]');
        matchStats.push({
            ...matchResult,
            date: matchResult.date.toISOString()
        });
        localStorage.setItem('matchStats', JSON.stringify(matchStats));
    }

    // Görev kontrolü için yardımcı fonksiyonlar
    getWinStreak() {
        const matchStats = JSON.parse(localStorage.getItem('matchStats') || '[]');
        let currentStreak = 0;
        
        // En son maçlardan geriye doğru kontrol et
        for (let i = matchStats.length - 1; i >= 0; i--) {
            if (matchStats[i].won) {
                currentStreak++;
            } else {
                break;
    ...
    checkMissionProgress(matchResult) {
        // Mevcut görev kontrolleri...
        missions.singleMatch.forEach(mission => {
            if (!mission.completed) {
                if (mission.type === 'single_win' && 
                    matchResult.opponent === mission.target && 
                    matchResult.won) {
                    mission.completed = true;
                    showMissionComplete(mission);
                }
                
                if (mission.type === 'win_margin' && 
                    matchResult.won && 
                    matchResult.margin >= mission.target) {
                    mission.completed = true;
                    showMissionComplete(mission);
                }
            }
        ...
        // Seri görevleri için güncel seriyi kontrol et
        const currentStreak = getWinStreak();
        missions.streak.forEach(mission => {
            if (!mission.completed && mission.type === 'win_streak') {
                mission.progress = currentStreak;
                if (currentStreak >= mission.target) {
                    mission.completed = true;
                    showMissionComplete(mission);
                }
            }
    ...
    showMissionComplete(mission) {
        ...
        notification.innerHTML = `
            ...
            <p>${mission.title}</p>
            <p class="reward">+${mission.reward.toLocaleString()}₺</p>
        `;
...
// Maç sonuçlarını kontrol et ve görevleri güncelle
function checkMissionProgress(matchResult) {
    matchHistory.push(matchResult);
    
    // Tek maç görevlerini kontrol et
    missions.singleMatch.forEach(mission => {
        if (mission.type === 'single_win' && 
            matchResult.opponent === mission.target && 
            matchResult.won) {
        ...
        if (mission.type === 'win_margin' && 
            matchResult.won && 
            (matchResult.myScore - matchResult.opponentScore) >= mission.target) {
            mission.completed = true;
        }
    });
    
    // Seri görevlerini kontrol et
    missions.streak.forEach(mission => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Filtre butonları için event listener'lar
    document.querySelectorAll('.filter-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            document.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('active'));
            e.target.classList.add('active');
            updateMissionBoard(e.target.dataset.filter);
        });
    });
});

// Görev verileri - Global olarak tanımla
const missions = {
    singleMatch: [
        {
            id: 'mission1',
            title: 'İlk Zafer',
            description: '1 maç kazan',
            reward: 250000,
            type: 'win_any',
            target: 1,
            completed: false
        },
        {
            id: 'mission011',
            title: 'deneme',
            description: 'denme lojamn\'ı yen',
            reward: 250000,
            type: 'single_win',
            target: 'denme lojamn',
            completed: false
        ...
        {
            id: 'mission_gsw',
            title: 'Warriors Mücadelesi',
            description: 'Golden State Warriors\'ı yen',
            reward: 1000000,
            type: 'single_win',
        ...
        {
            id: 'mission_cavs',
            title: 'Cavaliers Zaferi',
            description: 'Cleveland Cavaliers\'ı yen',
            reward: 900000,
            type: 'single_win',
        ...
        {
            id: 'mission_rockets',
            title: 'Rockets Galibiyeti',
            description: 'Houston Rockets\'ı yen',
            reward: 850000,
            type: 'single_win',
        ...
        },
        {
            id: 'mission_hawks',
            title: 'Hawks Avı',
            description: 'Atlanta Hawks\'ı yen',
            reward: 800000,
            type: 'single_win',
        ...
        {
            id: 'mission_clippers',
            title: 'Clippers Mücadelesi',
            description: 'Los Angeles Clippers\'ı yen',
            reward: 900000,
            type: 'single_win',
        ...
        {
            id: 'mission_spurs',
            title: 'Spurs Efsanesi',
            description: 'San Antonio Spurs\'u yen',
            reward: 1100000,
            type: 'single_win',
        ...
        {
            id: 'mission_blazers',
            title: 'Blazers Zaferi',
            description: 'Portland Trail Blazers\'ı yen',
            reward: 850000,
            type: 'single_win',
        ...
        {
            id: 'mission_suns',
            title: 'Suns Galibiyeti',
            description: 'Phoenix Suns\'ı yen',
            reward: 800000,
            type: 'single_win',
            target: 'Phoenix Suns',
            completed: false
        ...
        {
            id: 'mission_thunder',
            title: 'Thunder Mücadelesi',
            description: 'Oklahoma City Thunder\'ı yen',
            reward: 900000,
            type: 'single_win',
        ...
        },
        {
            id: 'mission_pelicans',
            title: 'Pelicans Avı',
            description: 'New Orleans Pelicans\'ı yen',
            reward: 800000,
            type: 'single_win',
        ...
        {
            id: 'mission_grizzlies',
            title: 'Grizzlies Zaferi',
            description: 'Memphis Grizzlies\'i yen',
            reward: 850000,
            type: 'single_win',
        ...
        {
            id: 'mission_mavericks',
            title: 'Mavericks Galibiyeti',
            description: 'Dallas Mavericks\'i yen',
            reward: 900000,
            type: 'single_win',
        ...
        {
            id: 'mission_heat',
            title: 'Heat Mücadelesi',
            description: 'Miami Heat\'i yen',
            reward: 950000,
            type: 'single_win',
            target: 'Miami Heat',
            completed: false
        },
        // ... Diğer takımlar devam ediyor ...
        {
            id: 'mission_dream1',
            title: 'Dream Team I Efsanesi',
            description: 'Dream Team I\'i yen',
            reward: 5000000,
            type: 'single_win',
            target: 'Dream Team I',
            completed: false
        ...
        {
            id: 'mission_dream2',
            title: 'Dream Team II Mücadelesi',
            description: 'Dream Team II\'yi yen',
            reward: 5500000,
            type: 'single_win',
            target: 'Dream Team II',
            completed: false
        },
        {
            id: 'mission_dream3',
            title: 'Dream Team III Zaferi',
            description: 'Dream Team III\'ü yen',
            reward: 6000000,
            type: 'single_win',
            target: 'Dream Team III',
            completed: false
        },
        {
            id: 'mission_hof',
            title: 'Hall of Fame Mücadelesi',
            description: 'Hall of Fame takımını yen',
            reward: 10000000,
            type: 'single_win',
            target: 'Hall of Fame',
            completed: false
        ...
        // Performans görevleri
        {
            id: 'mission_streak3',
            title: 'Kısa Seri',
            description: 'Art arda 3 maç kazan',
            reward: 2000000,
            type: 'win_streak',
            target: 3,
            currentProgress: 0,
        ...
        },
        {
            id: 'mission_streak5',
            title: 'Uzun Seri',
            description: 'Art arda 5 maç kazan',
            reward: 5000000,
            type: 'win_streak',
            target: 5,
            currentProgress: 0,
        ...
        },
        {
            id: 'mission_margin20',
            title: 'Fark Yaratıcı',
            description: '20 sayı farkla kazan',
            reward: 3000000,
            type: 'win_margin',
            target: 20,
            completed: false
        ...
        {
            id: 'mission_margin30',
            title: 'Dominant Performans',
            description: '30 sayı farkla kazan',
            reward: 5000000,
            type: 'win_margin',
            target: 30,
            completed: false
        },
        // Eksik NBA takımlarını ekle (missions.singleMatch array'ine ekle)
        {
            id: 'mission_pacers',
            title: 'Pacers Mücadelesi',
            description: 'Indiana Pacers\'ı yen',
            reward: 850000,
            type: 'single_win',
        ...
        {
            id: 'mission_jazz',
            title: 'Jazz Galibiyeti',
            description: 'Utah Jazz\'ı yen',
            reward: 900000,
            type: 'single_win',
            target: 'Utah Jazz',
            completed: false
        },
        {
            id: 'mission_kings',
            title: 'Kings Zaferi',
            description: 'Sacramento Kings\'i yen',
            reward: 850000,
            type: 'single_win',
        ...
        {
            id: 'mission_bucks',
            title: 'Bucks Mücadelesi',
            description: 'Milwaukee Bucks\'ı yen',
            reward: 1000000,
            type: 'single_win',
        ...
        {
            id: 'mission_raptors',
            title: 'Raptors Galibiyeti',
            description: 'Toronto Raptors\'ı yen',
            reward: 900000,
            type: 'single_win',
        ...
        {
            id: 'mission_bulls',
            title: 'Bulls Efsanesi',
            description: 'Chicago Bulls\'u yen',
            reward: 1200000,
            type: 'single_win',
        ...
        {
            id: 'mission_wizards',
            title: 'Wizards Zaferi',
            description: 'Washington Wizards\'ı yen',
            reward: 850000,
            type: 'single_win',
        ...
        {
            id: 'mission_nets',
            title: 'Nets Mücadelesi',
            description: 'Brooklyn Nets\'i yen',
            reward: 900000,
            type: 'single_win',
        ...
        {
            id: 'mission_knicks',
            title: 'Knicks Galibiyeti',
            description: 'New York Knicks\'i yen',
            reward: 950000,
            type: 'single_win',
        ...
        },
        {
            id: 'mission_76ers',
            title: '76ers Zaferi',
            description: 'Philadelphia 76ers\'ı yen',
            reward: 1000000,
            type: 'single_win',
        ...
        },
        {
            id: 'mission_hornets',
            title: 'Hornets Avı',
            description: 'Charlotte Hornets\'ı yen',
            reward: 800000,
            type: 'single_win',
        ...
        {
            id: 'mission_lakers',
            title: 'Lakers Efsanesi',
            description: 'Los Angeles Lakers\'ı yen',
            reward: 1500000,
            type: 'single_win',
        ...
        {
            id: 'mission_celtics',
            title: 'Celtics Efsanesi',
            description: 'Boston Celtics\'i yen',
            reward: 1300000,
            type: 'single_win',
    ...
    streak: [] // Diğer görev tipleri için
};
...
function createMissionCard(mission) {
    ...
    if (mission.type === 'win_streak') {
        ...
        progressText = `${currentProgress}/${mission.target}`;
    } else if (mission.type === 'single_win' || mission.type === 'win_margin' || mission.type === 'win_any') {
        currentProgress = mission.completed ? 1 : 0;
        progressText = `${currentProgress}/1`;
    }
    
    const progressPercent = (currentProgress / (mission.target || 1)) * 100;
    
    card.innerHTML = `
        <div class="mission-info">
            <div class="mission-title">${mission.title}</div>
            <div class="mission-description">${mission.description}</div>
            <div class="mission-reward">Ödül: $${mission.reward.toLocaleString()}</div>
        ...
        ${mission.completed && !mission.claimed ? 
            '<button class="claim-reward-btn" onclick="claimReward(\'' + mission.id + '\')">Ödülü Topla</button>' : 
            (mission.completed ? '<div class="mission-status completed">✓</div>' : '')}
    `;
    
    return card;
}

// Görev panosunu güncelleme
function updateMissionBoard(filter = 'all') {
    const missionsList = document.querySelector('.missions-list');
    if (!missionsList) {
        console.error('Missions list container not found');
        return;
    }

    missionsList.innerHTML = '';
    
    const allMissions = [...missions.singleMatch, ...missions.streak];
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('Page loaded, initializing missions...');
    loadMissionData();
    updateMissionBoard();
...
// Görev tamamlama ve ödül verme fonksiyonu
function completeMission(mission) {
    if (!mission.completed) {
        ...
        // Bildirim göster
        showNotification(`Görev tamamlandı! $${mission.reward.toLocaleString()} ödül kazandınız!`, 'success');
        
        // Görev panosunu güncelle
        updateMissionBoard();
    }
}

// Maç sonuçlarını kontrol eden fonksiyon
function checkMissionProgress(matchResult) {
    ...
    missions.singleMatch.forEach(mission => {
        if (!mission.completed) {
            switch(mission.type) {
                ...
                case 'single_win':
                    if (matchResult.opponent === mission.target && matchResult.won) {
                        mission.completed = true;
                        missionUpdated = true;
                        completeMission(mission);
                    }
                ...
                case 'win_margin':
                    if (matchResult.scoreDifference >= mission.target && matchResult.won) {
                        mission.completed = true;
                        missionUpdated = true;
                        completeMission(mission);
                    }
                ...
                case 'win_streak':
                    if (matchResult.won) {
                        mission.currentProgress = (mission.currentProgress || 0) + 1;
                        ...
                        mission.currentProgress = 0; // Seriyi sıfırla
                        missionUpdated = true;
                    }
                    break;
            }
...
// Topla butonunu ve fonksiyonlarını kaldır
function createTeamRow(team, index) {
    ...
    row.innerHTML = `
        ...
        <div class="team-info">
            <div class="team-name">${team.name}</div>
            <div class="team-power">Güç: ${calculateTeamPower(team)}</div>
        </div>
        <div class="team-actions">
            <button class="play-button" onclick="startMatch('${team.name}')">
                <i class="fas fa-play"></i> Oyna
            </button>
        </div>
    `;
...
function claimReward(missionId) {
    const mission = missions.singleMatch.find(m => m.id === missionId);
    if (mission && mission.completed && !mission.claimed) {
        ...
        // Bildirim göster
        showNotification(`Ödül toplandı! $${mission.reward.toLocaleString()} hesabınıza eklendi!`, 'success');
